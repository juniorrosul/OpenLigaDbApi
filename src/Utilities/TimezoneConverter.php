...
class TimezoneConverter
{
    ...
    public static function windowsToIana($windowsTimezone)
    {
        if (isset(self::$timezones[$windowsTimezone])) {
            $windowsTimezone = self::$timezones[$windowsTimezone];
        }

        if (!in_array($windowsTimezone, \DateTimeZone::listIdentifiers())) {
            throw new \Exception(sprintf("the timezone %s does not exists in your installation", $windowsTimezone));
        }
